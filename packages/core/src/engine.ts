  ...
  return {
    async loadModel(taskId: string, opts: LoadOptions = {}): Promise<LoadedModel> {
      ...
      if (entry.provider === 'builtin') {
        loaded.push({ taskId, modelId, byteLength: 0, cached: true })
        return {
          taskId, modelId, variant: variantName, bytes: new ArrayBuffer(0),
          byteLength: 0, cached: true, license: entry.license, provider: entry.provider,
        }
      }
