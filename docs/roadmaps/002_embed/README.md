...
Base of edge RAG: search over notes, files and history without a server. Downstream packages (`search`, `similarity`, `rerank`) reuse it. Alternatives: a 90MB transformers.js pull with manual tokenizer work, or an OpenAI key (server, cost, privacy).
