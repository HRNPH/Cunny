...
True streaming transcription (partial words while you speak, ~300ms latency) is what makes live captions and the offline assistant feel alive. Batch STT (006) chunked on VAD gives you 1–2s latency, acceptable, noticeably worse. This module is the **highest technical-risk item in Phase 2** and is deliberately scheduled last with a fallback story.
...
- The captions orchestration (019, this is the engine, not the pipeline), multilingual streaming (v2), diarization (033 composes)
