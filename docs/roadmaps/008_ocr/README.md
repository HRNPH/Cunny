...
This is the **first pure-ONNX multi-model pipeline** in the SDK (no mediapipe, no transformers.js): it validates our own pre/post-processing competence end to end, DB postprocess, CTC decode, reading-order layout. Hardest module in Phase 1; deliberately last.
