...
- **ONNX Runtime Web and MediaPipe runtimes are wrapped by provider packages** (`@cunny-ai/provider-mediapipe`, `@cunny-ai/provider-onnx`), not imported by tasks or core. This is the dedupe boundary: one pin, one 3–11MB wasm download per browser, shared across every task.
